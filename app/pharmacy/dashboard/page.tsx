...
import { ArrowLeft, Package, Clock, CheckCircle, TrendingUp, RefreshCw, Building2, Volume2, VolumeX, Bell, Users } from "lucide-react"
...
export default function PharmacyDashboardPage() {
  ...
  useEffect(() => {
    const checkPharmacy = async () => {
      console.log('[v0] Pharmacy Dashboard: Verifying authenticated access...')
      
      // SECURE: Only the logged-in pharmacy owner can access their dashboard
      const protection = await protectDashboard('pharmacy')
      
      if (!protection.isAuthorized) {
        console.error('[v0] Pharmacy Dashboard: Access DENIED')
        router.push("/auth/login")
        return
      }

      if (!protection.provider) {
        console.error('[v0] Pharmacy Dashboard: Provider not found')
        router.push("/")
        return
      }

      console.log('[v0] Pharmacy Dashboard: AUTHORIZED -', protection.provider.name)
      
      // Store owner email for team management
      localStorage.setItem('shusto_pharmacy_owner_email', protection.provider.email)
      ...
      fetchLocalOrders(protection.provider.id)
      setLoading(false)
    }
