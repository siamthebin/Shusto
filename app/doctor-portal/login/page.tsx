...
export default function DoctorLoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[v0] Saved doctor email to localStorage:", email)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/doctor-portal/dashboard`,
        },
      })
      if (error) throw error
    } catch (error: unknown) {
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="shadow-xl">
            <CardHeader className="space-y-3">
              <CardTitle className="text-3xl text-center font-bold">ডাক্তার লগইন</CardTitle>
              <CardDescription className="text-center text-base">আপনার অ্যাকাউন্টে লগইন করুন</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <form onSubmit={handleEmailLogin} className="flex flex-col gap-6">
                ...
                <Button
                  type="button"
                  size="lg"
                  className="w-full bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-200 shadow-md h-14 text-lg font-semibold"
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-600">
                    নতুন ডাক্তার?{" "}
                    <Link href="/doctor-portal/register" className="text-emerald-600 font-semibold hover:underline">
                      রেজিস্টার করুন
                    </Link>
                  </p>
                </div>
