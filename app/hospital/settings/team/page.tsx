...
export default function HospitalTeamPage() {
  ...
  useEffect(() => {
    const loadTeam = async () => {
      try {
        const hospitalData = localStorage.getItem("shusto_hospital_logged_in")
        const ownerData = localStorage.getItem("shusto_hospital_owner_email")

        if (!hospitalData || !ownerData) {
        ...
        const response = await fetch(
          `/api/provider/roles?provider_id=${hospital.id}&provider_type=hospital&user_email=${ownerData}`
        )
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    setError("")
    setSuccess("")
    if (!hospital || !newMemberEmail) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        const reloadResponse = await fetch(
          `/api/provider/roles?provider_id=${hospital.id}&provider_type=hospital&user_email=${ownerEmail}`
        )
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <form onSubmit={handleAddMember} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="manager">Manager (Manage beds/patients)</option>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              <p><strong>Viewer:</strong> Can only view patient data and analytics</p>
              <p><strong>Manager:</strong> Can manage patient records and beds</p>
