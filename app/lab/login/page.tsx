...
export default function LabLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'এই email দিয়ে কোনো ল্যাব পাওয়া যায়নি')
        setLoading(false)
        return
      }

      localStorage.setItem('labEmail', email)
      localStorage.setItem('labData', JSON.stringify(data.lab))
    ...
    <div className="min-h-screen bg-gradient-to-b from-purple-50 to-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-purple-700 mb-2">ল্যাব ড্যাশবোর্ড</h1>
        ...
        <p className="text-center text-sm text-gray-600 mt-4">
          ...
          <Link href="/lab/register" className="text-purple-600 hover:underline font-semibold">
            এখানে ক্লিক করুন
          </Link>
        </p>
      </div>
