...
export default function AmbulanceDashboard() {
  ...
  useEffect(() => {
    const loadAmbulanceData = async () => {
      try {
        console.log('[v0] Ambulance Dashboard: Verifying authenticated access...')
        
        // SECURE: Only the logged-in ambulance owner can access their dashboard
        const protection = await protectDashboard('ambulance')
        
        if (!protection.isAuthorized) {
          console.error('[v0] Ambulance Dashboard: Access DENIED')
        ...
        if (!protection.provider) {
          setError('অ্যাম্বুলেন্স তথ্য খুঁজে পাওয়া যায়নি')
          return
        }

        console.log('[v0] Ambulance Dashboard: AUTHORIZED -', protection.provider.name)
        setAmbulance({
        ...
        console.error('[v0] Ambulance Dashboard: Error:', err)
