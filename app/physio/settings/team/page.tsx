...
export default function PhysioTeamPage() {
  ...
  useEffect(() => {
    const loadTeam = async () => {
      try {
        const physioData = localStorage.getItem("shusto_physio_logged_in")
        const ownerData = localStorage.getItem("shusto_physio_owner_email")

        if (!physioData || !ownerData) {
          router.push("/physio/login")
          return
        }

        const physio = JSON.parse(physioData)
        setPhysio(physio)
        setOwnerEmail(ownerData)

        const response = await fetch(
          `/api/provider/roles?provider_id=${physio.id}&provider_type=physio&user_email=${ownerData}`
        )
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    setError("")
    setSuccess("")
    if (!physio || !newMemberEmail) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        const reloadResponse = await fetch(
          `/api/provider/roles?provider_id=${physio.id}&provider_type=physio&user_email=${ownerEmail}`
        )
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">{physio?.name}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <form onSubmit={handleAddMember} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="manager">Manager (Manage sessions)</option>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              <p><strong>Viewer:</strong> Can only view patient data and therapy sessions</p>
              <p><strong>Manager:</strong> Can manage patient sessions and schedules</p>
